...
def fetch_spc_geojson(layer_id: int):
    ...
    headers = {"User-Agent": "(ChaseBriefGenerator/1.7, contact: stormbriefs@weatherops.org)"}
    try:
...
st.caption(f"Tap pins to toggle offices. Max allowed: {max_allowed_cwas}. Selected: {len(st.session_state.selected_cwas)}/{max_allowed_cwas}")
...
# Detect changes initiated purely from dropdown
parsed_from_dropdown = [OPTION_TO_CWA[opt] for opt in selected_options]
if set(parsed_from_dropdown) != set(st.session_state.selected_cwas):
    st.session_state.selected_cwas = parsed_from_dropdown
    st.rerun()
...
# 3. Precision-Targeted Office Markers (slightly larger for clean mobile tapping)
for cwa, data in CHASE_CWAS.items():
...
# Precision Mobile Click Handler
if map_state and map_state.get("last_clicked"):
    current_click = (round(map_state["last_clicked"]["lat"], 3), round(map_state["last_clicked"]["lng"], 3))
    
    # Process only when it represents a genuinely new tap
    if current_click != st.session_state.last_processed_click:
        ...
        # Tight 0.85-degree radius prevents phantom snapping to southern offices
        closest_cwa = None
        min_dist = 0.85
        for cwa, data in CHASE_CWAS.items():
...
if st.session_state.selected_cwas:
    ...
    st.warning("Please select at least one office on the map above or from the dropdown.")
